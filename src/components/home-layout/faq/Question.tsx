...
const Question: React.FC<QuestionProps> = ({ question, answer, open, onToggle }) => {
  ...
  return (
    <div className="w-full rounded-xl border border-white/10 bg-white/5 p-4 backdrop-blur-md">
      <button
        type="button"
        aria-expanded={open}
      ...
      >
        <div id={answerId} className="overflow-hidden pr-1">
          <p className={`mt-3 text-sm leading-6 text-white/80 transition-all duration-300 ${open ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-1'}`}>
            {answer}
          </p>
        </div>
      </div>
